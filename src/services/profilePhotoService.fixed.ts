...
export const deleteProgressPhoto = async (photoId: string): Promise<boolean> => {
  try {
    ...
    console.log('Extraindo caminho de arquivo da URL:', photo.photo_url);
    const url = new URL(photo.photo_url);
    
    // Analisar a URL para extrair o caminho do arquivo
    let filePath = '';
    
    // Para URLs do tipo /storage/v1/object/public/bucket/path
    if (url.pathname.includes('/object/public/')) {
      const pathPart = url.pathname.split('/object/public/')[1];
      if (pathPart) {
        filePath = pathPart;
      }
    } 
    // Para URLs do tipo /storage/v1/object/sign/bucket/path
    else if (url.pathname.includes('/object/sign/')) {
      const pathPart = url.pathname.split('/object/sign/')[1];
      if (pathPart) {
        filePath = pathPart;
      }
    }
    // Outros formatos de URL - tenta extrair a parte final
    else {
      const pathParts = url.pathname.split('/');
      if (pathParts.length >= 2) {
        filePath = `${user.id}/${pathParts[pathParts.length - 1]}`;
      }
    }
    ...
    if (!filePath) {
      console.error('Não foi possível extrair o caminho do arquivo da URL:', photo.photo_url);
      return false;
    }
