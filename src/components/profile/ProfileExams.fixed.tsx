...
const ProfileExams: React.FC<ProfileExamsProps> = ({ showNutritionRecommendations = false }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      try {
        ...
        const success = await processExamWithContent(examId, fileContent, detectedType || examType, openaiKey);
        ...
        const success = await processExamWithContent(examId, minimalContent, examType, openaiKey);
  ...
  const processExamWithContent = async (examId: string, fileContent: string, examType: string, customApiKey?: string): Promise<boolean> => {
    try {
      ...
      const analysisResult = await analyzeExamWithAI(
        ...
        examType,
        {
          forceType: Boolean(examType), // Se o usuário selecionou um tipo, forçar o uso
          extractNumericValues: true,
          forceValuesExtraction: true
        },
        customApiKey
      );
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <Button 
                      ...
                      className="text-xs"
                    >
                      {showApiKeyInput ? 'Ocultar' : 'Mostrar'}
                    </Button>
                  </div>
                  ...
                  {!showApiKeyInput && (
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Opcional. Deixe em branco para usar a chave padrão do sistema.
                    </p>
                  )}
                </div>
