...
export const processAnalysisText = (text: string): React.ReactNode => {
  ...
  // Remover qualquer espaço extra no início e fim do texto
  let cleanText = text.trim();
  
  // Remover quebras de linha repetidas
  cleanText = cleanText.replace(/\n\s*\n/g, '\n');
  
  // Tratar os marcadores ### e formatar o texto automaticamente
  ...
  if (sections.length <= 1) {
    // Se não houver seções, tratar o texto inteiro
  ...
  return (
    <div className="space-y-6">
      {sections.map((section, index) => {
        ...
        // Extrair o título da seção (primeira linha)
        const lines = section.trim().split('\n');
        const title = lines[0].trim();
        const content = lines.slice(1).join('\n');
        
        // Determinar ícone e cor com base no título
        const { icon: SectionIcon, color } = getSectionIconAndColor(title);
        
        return (
          <div key={index} className={`p-4 rounded-lg border border-${color}-100 bg-${color}-50 dark:bg-gray-800 dark:border-gray-700`}>
            <div className="flex items-center gap-2 mb-3">
              <SectionIcon className={`h-5 w-5 text-${color}-600 dark:text-${color}-400`} />
              <h3 className={`font-medium text-${color}-800 dark:text-${color}-300`}>{title}</h3>
            </div>
...
// Função auxiliar para determinar ícone e cor com base no título da seção
const getSectionIconAndColor = (title: string): { icon: React.FC<{ className?: string }>, color: string } => {
